...
// Cache batch-folder lookups for the life of the process to avoid repeat API calls
const batchFolderCache = new Map();

// Finds (or creates) a subfolder under the root assignments folder for a given batch
...
// Uploads a file buffer into the given batch's folder, makes it link-viewable,
// and returns { fileId, webViewLink }
async function uploadFileToBatchFolder({ batchId, batchName, fileName, mimeType, buffer }) {
  ...
  // Anyone with the link can view — needed so students/faculty can open it
  // without needing to be individually shared on the file.
...
// Cache assignment-submission-folder lookups too
const assignmentFolderCache = new Map();

// Finds (or creates) a "<Assignment Title> - Submissions" subfolder inside
// the batch's folder, so each assignment's student submissions stay grouped.
...
// Uploads a student's submission file into that assignment's submissions folder
